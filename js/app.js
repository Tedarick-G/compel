...
function buildUnifiedUnmatched({Uc,Ut,Ud}){
  ...
  // ✅ Compel unmatched: ürün adı + ürün kodu
  for(const r of(Uc||[])){
    const bDisp=String(r["Marka"]||"").trim(),bNorm=normBrand(bDisp||r._bn||"");
    const grp=getGrp(bNorm,bDisp);if(!grp)continue;
    const nm=String(r["Ürün Adı (Compel)"]||r["Compel Ürün Adı"]||"").trim();if(!nm)continue;
    const code=String(r["Ürün Kodu (Compel)"]||r["Compel Ürün Kodu"]||"").trim(); // ✅ NEW
    grp.c.push({name:nm,code,link:r._clink||"",stokRaw:r._s1raw??""})
  }

  // ✅ T-Soft unmatched: ürün adı + ürün kodu (supplier ürün kodu)
  for(const r of(Ut||[])){
    const bDisp=String(r["Marka"]||"").trim(),bNorm=normBrand(r._bn||bDisp||"");
    const grp=getGrp(bNorm,bDisp);if(!grp)continue;
    const nm=String(r["T-Soft Ürün Adı"]||"").trim();if(!nm)continue;
    const code=String(r._sup||r["T-Soft Ürün Kodu"]||"").trim(); // ✅ NEW
  ...
  // ✅ Aide unmatched: ürün adı + ürün kodu (depot stok kodu)
  for(const r of(Ud||[])){
    const bDisp=String(r["Marka"]||"").trim(),bNorm=normBrand(r._bn||bDisp||"");
    const grp=getGrp(bNorm,bDisp);if(!grp)continue;
    const nm=String(r["Depo Ürün Adı"]||r["Aide Ürün Adı"]||"").trim();if(!nm)continue;
    const code=String(r["Aide Ürün Kodu"]||"").trim(); // ✅ NEW
  ...
  for(const grp of brandArr){
    ...
    for(let i=0;i<n;i++){
      ...
      out.push({
        ...
        // ✅ NEW: Kod kolonları dolu gelecek
